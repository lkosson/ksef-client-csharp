...
public class CredentialsGrantTests: TestBase
{
    [Theory]
    ...
    public async Task GivenDelegatedByOwnerIsAuthenticated_WhenGrantInvoiceIssuingPermissionToEntity_ThenPermissionIsConfirmed(string identyficator, StandardPermissionType[] permissions)
    {
        ...
        var grantPermissionsResponse = await PermissionsUtils.GrantCredentialsManageToDelegateAsync(ksefClient,
                authToken, nipWhichWillDelegatePermissions);
